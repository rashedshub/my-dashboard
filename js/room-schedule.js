...
// ── Render today's bookings across all rooms ──────────────────────────────────
function renderToday() {
  const todayKey  = dateKey(new Date());
  const nowMins   = new Date().getHours()*60 + new Date().getMinutes();
  const labelEl   = el("todayDateLabel");
  const gridEl    = el("todayGrid");
  if (!gridEl) return;

  if (labelEl) {
    labelEl.textContent = new Date().toLocaleDateString("en-US",{
      weekday:"long", year:"numeric", month:"long", day:"numeric"
    });
  }

  if (!rooms.length) {
    gridEl.innerHTML = `<div style="padding:20px;color:#94a3b8;font-size:0.875rem;">No rooms configured.</div>`;
    return;
  }

  gridEl.innerHTML = rooms.map(room => {
    const roomBks = bookings
      .filter(b => b.roomId === room.id && b.date === todayKey)
      .sort((a,b) => a.startTime.localeCompare(b.startTime));

    const slotsHtml = roomBks.length === 0
      ...
      : roomBks.map(b => {
          ...
          return `<div class="today-slot${isNow?" active-now":""}">
            <div class="today-slot-time">${fmtTime(bs)} – ${fmtTime(be)}</div>
            <div class="today-slot-title">${b.title}</div>
            <div class="today-slot-by">👤 ${b.bookedByName||"—"}</div>
          </div>`;
        }).join("");
    ...
    return `<div class="today-room">
      <div class="today-room-head">
        ...
        ${room.name} <span style="font-weight:400;color:#94a3b8;">(${room.capacity})</span>
      </div>
      ${slotsHtml}
    </div>`;
  }).join("");
